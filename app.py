...
# File IDs are extracted from your share links automatically.
# On Render/Railway/Heroku you can override via environment variables.
GDRIVE_MODEL_IDS = {
    ALPHABET_MODEL_PATH : os.environ.get('GDRIVE_ALPHABET_RF',  '1jfUt7nxckePnygjYjsKv7Mr-Vcs7S9bS'),
    DIGIT_MODEL_PATH    : os.environ.get('GDRIVE_DIGIT_RF',     '1tCn3dqKivMLek2mSpepg6kVrpkowy8b6'),
    PCA_ALPHABET_PATH   : os.environ.get('GDRIVE_PCA_ALPHABET', '1ncAkwgX3tRwKsf51PLw9PlLK8lMAPRl4'),
    PCA_DIGIT_PATH      : os.environ.get('GDRIVE_PCA_DIGIT',    '1oSvYmi8OlLAUkB_eV_JlJrof7I4Atmhu'),
    HMM_ALPHABET_PATH   : os.environ.get('GDRIVE_HMM_ALPHABET', '1uXNJV5FaDD3SCx1lqHixHguaAExnYxwS'),
    HMM_DIGIT_PATH      : os.environ.get('GDRIVE_HMM_DIGIT',    '1kMJ3VAhpt1UNimcyFKl6BqskVsWyrVPl'),
}

def extract_file_id(id_or_url: str) -> str:
    """
    Accept either a raw File ID or a full Google Drive share URL and
    always return just the File ID.
    e.g. 'https://drive.google.com/file/d/1ABC.../view?usp=sharing' -> '1ABC...'
    """
    if not id_or_url:
        return ''
...
def download_from_gdrive(file_id_or_url: str, dest_path: str) -> bool:
    """Download a file from Google Drive. Accepts a File ID or a share URL."""
    ...
    print(f"  ⬇ Downloading {dest_path} from Google Drive (id={file_id}) ...")
    ...
    try:
        ...
        # Large files get a virus-scan warning page — confirm it
        token = None
        ...
        if response.status_code != 200:
            print(f"  ✗ Failed (HTTP {response.status_code}) for {dest_path}")
            return False
...
def download_all_models():
    ...
    for local_path, file_id_or_url in GDRIVE_MODEL_IDS.items():
        if file_id_or_url:
            download_from_gdrive(file_id_or_url, local_path)
        else:
            print(f"  – {local_path}: no Drive ID set, skipping.")
    print("📥 Model download step complete.\n")

# ==================== GLOBAL VARIABLES ====================
# NOTE: Camera is handled by the browser (WebRTC).
# The server receives frames via POST /process_frame.
...
def predict_from_landmarks(landmark_sequence, mode):
    ...
    try:
        ...
        if mode == "alphabet":
            predicted_char = chr(int(prediction) + ord('A')) if 0 <= prediction <= 25 \
                             else chr(int(prediction % 26) + ord('A'))
        else:
...
def process_frame_data(frame):
    ...
    if is_detecting and current_prediction != "-":
        ...
        cv2.putText(frame, f"RF: {current_confidence:.1%}",
                    (w - 150, text_size[1] + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        cv2.putText(frame, f"HMM: {current_hmm_score:.1f}",
                    (w - 150, text_size[1] + 85), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    ...
    cv2.putText(frame, f"Buffer: {len(landmark_buffer)}/15",
...
def process_frame():
    ...
    try:
        ...
        img_data = base64.b64decode(
            data['frame'].split(',')[1] if ',' in data['frame'] else data['frame']
        )
